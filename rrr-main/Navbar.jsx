/* Mr Gleamz — Navbar. Desktop: floating pill on scroll + mega-menu. Mobile: full-screen dark menu. */
...
const SERVICE_MENU = [
  {
    group: "Cleaning",
    items: [
      { icon: "wash",                       label: "Basic Wash & Maintenance", price: "from £30",  bg: "#2A2214", iconColor: "#C9A24B" },
      { icon: "airline_seat_recline_extra", label: "Interior Detail",          price: "from £30",  bg: "#2A2214", iconColor: "#C9A24B" },
      { icon: "local_car_wash",             label: "Exterior Detail",          price: "from £20",  bg: "#2A2214", iconColor: "#C9A24B" },
    ],
  },
  {
    group: "Restoration",
    items: [
      { icon: "auto_fix",                   label: "Full Valet / Detail",      price: "from £60",  bg: "#2A2214", iconColor: "#C9A24B" },
      { icon: "cleaning_services",          label: "Deep Clean / Pre-Sale",    price: "from £60",  bg: "#2A2214", iconColor: "#C9A24B" },
      { icon: "lens_blur",                  label: "Machine Polish",           price: "from £100", bg: "#2A2214", iconColor: "#C9A24B" },
    ],
  },
  {
    group: "Protection",
    items: [
      { icon: "shield",                     label: "Ceramic Coating",          price: "from £150", bg: "#2A2214", iconColor: "#C9A24B" },
      { icon: "workspace_premium",          label: "Masterclass",              price: "from £200", bg: "#2A2214", iconColor: "#C9A24B" },
    ],
  },
];
  ...
  return (
    <>
      ...
      <div style={{ position:"fixed",top:0,left:0,right:0,zIndex:100,display:"flex",justifyContent:"center",padding:scrolled?"12px 24px":"0",transition:"padding 0.3s ease",pointerEvents:"none" }}>
        <header className="scheme-3" style={{ pointerEvents:"all",display:"flex",alignItems:"center",width:"100%",maxWidth:scrolled?"860px":"100%",minHeight:scrolled?"3.2rem":"4.5rem",padding:scrolled?"0 20px":"0 5%",borderRadius:scrolled?999:0,border:scrolled?"1px solid rgba(255,255,255,0.10)":"none",borderBottom:scrolled?"1px solid rgba(255,255,255,0.10)":"1px solid var(--scheme-border)",background:"#0A0A0A",boxShadow:scrolled?"0 4px 24px rgba(0,0,0,0.45)":"none",transition:"all 0.35s cubic-bezier(0.4,0,0.2,1)",overflow:"visible" }}>
          <div className="rr-nav-inner" style={{ display:"grid",gridTemplateColumns:"1fr auto 1fr",alignItems:"center",width:"100%",position:"relative" }}>
            ...
            <nav className="rr-desktop" style={{ gap:scrolled?20:28,justifyContent:"center" }}>
              {links.map((l) => l === "Services" ? (
                <div key={l} style={{ position:"relative",display:"flex",alignItems:"center" }} onMouseEnter={()=>{ setHoveredLink(l); openMega(); }} onMouseLeave={()=>{ setHoveredLink(null); closeMega(); }}>
                  <a href="#" onClick={(e)=>{ e.preventDefault(); handleNav(l); }} style={{ fontFamily:"var(--font-body)",fontSize:scrolled?14:15,fontWeight:500,textDecoration:"none",color:active===l||hoveredLink===l?"#FFFFFF":"#A8A29A",borderBottom:active===l?"2px solid var(--accent)":"2px solid transparent",paddingBottom:2,display:"inline-flex",alignItems:"center",gap:4,transition:"color 0.15s ease" }}>
                    {l} <span style={{ fontSize:10,opacity:0.6,marginTop:1 }}>▾</span>
                  </a>
                  {megaOpen && (
                    <div style={{ position:"absolute",top:"calc(100% + 24px)",left:"50%",transform:"translateX(-50%)",background:"#16130D",borderRadius:14,boxShadow:"0 8px 40px rgba(0,0,0,0.55)",border:"1px solid #2A2418",padding:"20px 24px 16px",display:"grid",gridTemplateColumns:"repeat(3,190px)",gap:"0 24px",minWidth:600,zIndex:200,animation:"fadeInDown 0.15s ease" }}>
                      {SERVICE_MENU.map((col) => (
                        <div key={col.group}>
                          <p style={{ fontSize:11,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:"#A8A29A",margin:"0 0 10px" }}>{col.group}</p>
                          {col.items.map((item) => (
                            <a key={item.label} href="#" onClick={(e)=>{ e.preventDefault(); handleNav("Services"); setMegaOpen(false); }} style={{ display:"flex",alignItems:"center",gap:10,padding:"7px 8px",borderRadius:8,textDecoration:"none",marginBottom:2,transition:"background 0.12s ease" }} onMouseEnter={(e)=>e.currentTarget.style.background="#2A2418"} onMouseLeave={(e)=>e.currentTarget.style.background="transparent"}>
                              ...
                              <span style={{ fontSize:14,fontWeight:600,color:"#FFFFFF" }}>{item.label}</span>
                            </a>
                          ))}
                        </div>
                      ))}
            ...
            <div className="rr-hamburger" style={{ alignItems:"center" }}>
              <button onClick={()=>setMobileOpen(o=>!o)} aria-label="Menu" style={{ background:"none",border:"none",cursor:"pointer",padding:8,display:"flex",flexDirection:"column",gap:5,alignItems:"center",justifyContent:"center" }}>
                <span style={{ display:"block",width:22,height:2,background:"#FFFFFF",borderRadius:2,transform:mobileOpen?"translateY(7px) rotate(45deg)":"none",transition:"transform 0.22s ease" }} />
                <span style={{ display:"block",width:22,height:2,background:"#FFFFFF",borderRadius:2,opacity:mobileOpen?0:1,transition:"opacity 0.15s ease" }} />
                <span style={{ display:"block",width:22,height:2,background:"#FFFFFF",borderRadius:2,transform:mobileOpen?"translateY(-7px) rotate(-45deg)":"none",transition:"transform 0.22s ease" }} />
              </button>
            </div>
          </div>
      ...
      {mobileOpen && (
        <div data-mobile-menu style={{ position:"fixed",inset:0,zIndex:200,display:"flex",flexDirection:"column",animation:"rrMenuIn 0.18s ease",background:"#0A0A0A" }}>
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 20px 16px" }}>
            ...
            <button onClick={()=>setMobileOpen(false)} aria-label="Close" style={{ background:"none",border:"none",cursor:"pointer",fontSize:22,color:"#FFFFFF",lineHeight:1,padding:"4px 8px",fontWeight:300 }}>✕</button>
          </div>

          <div style={{ flex:1,overflowY:"auto",paddingBottom:72 }}>
            {SERVICE_MENU.map((group) => (
              <div key={group.group} style={{ marginBottom:8 }}>
                <p style={{ fontSize:11,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--accent)",margin:0,padding:"10px 20px 6px" }}>{group.group}</p>
                <div style={{ background:"#16130D" }}>
                  {group.items.map((item, i) => (
                    <a key={item.label} href="#" onClick={(e)=>{ e.preventDefault(); handleNav("Services"); }} style={{ display:"flex",alignItems:"center",gap:14,padding:"12px 20px",textDecoration:"none",borderBottom:i<group.items.length-1?"1px solid rgba(255,255,255,0.07)":"none" }}>
                      ...
                      <span style={{ flex:1,fontSize:16,fontWeight:500,color:"#FFFFFF",fontFamily:"var(--font-body)" }}>{item.label}</span>
                      <span style={{ fontSize:13,color:"#A8A29A",fontWeight:400,whiteSpace:"nowrap" }}>{item.price}</span>
                    </a>
                  ))}
                </div>
              </div>
            ...
            {SECONDARY_LINKS.map((l) => (
              <a key={l} href="#" onClick={(e)=>{ e.preventDefault(); handleNav(l); }} style={{ display:"block",padding:"13px 20px",fontSize:16,fontWeight:400,color:"#FFFFFF",textDecoration:"none",fontFamily:"var(--font-body)" }}>{l}</a>
            ))}
          </div>

          <div style={{ position:"absolute",bottom:0,left:0,right:0,height:64,display:"flex",borderTop:"1px solid rgba(255,255,255,0.12)",background:"#0A0A0A" }}>
            <a href="tel:+447462885070" style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:8,textDecoration:"none",color:"#FFFFFF",fontSize:15,fontWeight:500,borderRight:"1px solid rgba(255,255,255,0.12)",fontFamily:"var(--font-body)" }}>
